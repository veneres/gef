...
def main():
    with open("config.json") as f:
        ...
        n_pairs = config_dict["bench_n_inter"]
        bench_inter_out = config_dict["bench_inter_out"]
    ...
    for inter in tqdm(real_interactions):
        ...
        for strategy in ["h_stat", "count_path", "gain_path", "pair_gain"]:
            ...
            explainer.explain(forest_to_explain, lam_search_space=[0.1, 1])

            # Get stats
            res_map[strategy].append(get_ap(explainer, inter))
            res_ap_3[strategy].append(get_p_k(explainer, inter, 3))
            res_ap_5[strategy].append(get_p_k(explainer, inter, 5))

    stats_res = {"map": res_map, "ap_3": res_ap_3, "ap_5": res_ap_5, "interactions": real_interactions}

    with open(bench_inter_out, 'wb') as f:
